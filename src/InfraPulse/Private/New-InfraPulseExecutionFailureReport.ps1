function New-InfraPulseExecutionFailureReport {
    ...
    return New-InfraPulseReport -RequestedComputerName $RequestedComputerName -ComputerName $ComputerName -Inventory $null -Results @($result) -DurationMs $DurationMs -Tags $Tags -RunId $RunId -ConfigurationFingerprint $ConfigurationFingerprint -ConfigurationSource $ConfigurationSource
}
